<script lang="ts">
	...
	let deletionCounts = $derived(project.deletion_counts ?? {
		agents: project.agent_ids.length,
	...
	let deleteConfirmed = $derived(deleteConfirmation === project.name);
		...
		{#if confirmingDelete}
			<div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-5" aria-busy={deleting}>
				<div class="rounded-xl border border-destructive/30 bg-destructive/5 p-4">
					<p class="text-sm font-medium text-foreground">This permanently deletes “{project.name}” and cannot be undone.</p>
				...
				{/if}
				<div>
					<label for="project-delete-confirmation" class="text-xs font-medium">Type <span class="font-semibold">{project.name}</span> to confirm</label>
			...
			<div class="flex justify-end gap-2 border-t border-border px-5 py-4">
				<button type="button" onclick={() => { confirmingDelete = false; deleteConfirmation = ''; error = ''; }} disabled={deleting} class="rounded-lg border border-border px-4 py-2 text-sm hover:bg-accent disabled:opacity-40">Back</button>
		...
		{:else}
			<form onsubmit={(event) => { event.preventDefault(); void save(); }}>
				...
				<div class="flex flex-wrap items-center justify-between gap-3 border-t border-border px-5 py-4">
					<button type="button" onclick={() => { confirmingDelete = true; deleteConfirmation = ''; error = ''; setTimeout(() => deleteInput?.focus()); }} class="rounded-lg px-3 py-2 text-sm font-medium text-destructive hover:bg-destructive/10">Delete project</button>
