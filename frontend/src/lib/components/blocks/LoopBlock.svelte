...
<div class="group rounded-lg bg-amber-950/10 loop-animated-border">
	...
	<div class="flex items-center gap-2 px-3 py-2">
		...
		{/if}
		{#if !compact}
			<button onclick={ontoggle} aria-label={expanded ? 'Collapse loop' : 'Expand loop'} class="text-muted-foreground hover:text-foreground">
			...
			<button onclick={onremove} aria-label="Remove loop" class="text-muted-foreground/30 hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity">
	...
	{#if expanded && !compact}
		<div class="border-t border-dashed border-amber-600/30 px-3 py-2 space-y-2">
			<div class="grid grid-cols-2 gap-2">
				<div>
					<label class="block text-[10px] font-medium text-muted-foreground mb-1">ITERATING
						<input type="text" value={overVar} oninput={(e) => onupdate({ overVar: e.currentTarget.value })}
							class="mt-1 w-full rounded border border-input bg-background px-2 py-1.5 text-xs font-mono" placeholder="@step.entities" />
					</label>
				</div>
				<div>
					<label class="block text-[10px] font-medium text-muted-foreground mb-1">AS
						<input type="text" value={asVar} oninput={(e) => onupdate({ asVar: e.currentTarget.value })}
							class="mt-1 w-full rounded border border-input bg-background px-2 py-1.5 text-xs font-mono" placeholder="entity" />
					</label>
				</div>
			</div>
		</div>
