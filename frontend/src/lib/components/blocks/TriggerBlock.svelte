...
<div class="group rounded-lg border border-border/60 bg-emerald-950/20 border-l-[3px] border-l-emerald-500">
	...
	<div class="flex items-center gap-2 px-3 py-2">
		...
		<button onclick={onremove} aria-label="Remove trigger" class="text-muted-foreground/30 hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity">
	...
	{#if expanded}
		<div class="border-t border-border/40 px-3 py-3 space-y-3">
			<div class="grid grid-cols-2 gap-2">
				<div>
					<label class="block text-[10px] font-medium text-muted-foreground mb-1">CHANNEL
						<input type="text" value={channel} oninput={(e) => onupdate({ channel: e.currentTarget.value })}
							class="mt-1 w-full rounded border border-input bg-background px-2 py-1.5 text-xs" placeholder="#channel-name" />
					</label>
				</div>
				<div>
					<label class="block text-[10px] font-medium text-muted-foreground mb-1">EVENT
						<input type="text" value={event} oninput={(e) => onupdate({ event: e.currentTarget.value })}
							class="mt-1 w-full rounded border border-input bg-background px-2 py-1.5 text-xs" placeholder="message_received" />
					</label>
				</div>
			</div>
			<div>
				<label class="block text-[10px] font-medium text-muted-foreground mb-1">CONNECTOR
