<script lang="ts">
	...
	async function loadAll(scroll = false) {
		try {
			...
			if (!taskAgent) taskAgent = participantAgents[0]?.id ?? '';
	...
	function selectWorkflow(id: string) {
		...
		for (const [name, input] of Object.entries(definition?.inputs ?? {})) {
			...
			else if (input.type === 'agent') workflowValues[name] = participantAgents[0]?.id ?? '';
