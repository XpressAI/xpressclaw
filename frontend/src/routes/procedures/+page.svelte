						...
						<div class="group flex items-center gap-1">
							...
							{#if confirmDeleteId === sop.id}
								<div class="flex items-center gap-1.5 pr-1 shrink-0">
									<button onclick={() => deleteProcedure(sop.id)}
										class="rounded border border-destructive/50 bg-destructive/10 px-2 py-0.5 text-[10px] font-medium text-destructive hover:bg-destructive/20 transition-colors">
										Delete
									</button>
									<button onclick={() => (confirmDeleteId = null)}
										class="rounded border border-border px-2 py-0.5 text-[10px] text-muted-foreground hover:bg-accent transition-colors">
										Cancel
									</button>
								</div>
							{:else}
								<button onclick={() => (confirmDeleteId = sop.id)}
									class="opacity-0 group-hover:opacity-100 shrink-0 rounded p-1 text-muted-foreground/40 hover:text-destructive hover:bg-destructive/10 transition-colors">
