...
<div class="workspace-scroll-y h-full">
	{#if loading}
		<div class="p-6 text-sm text-muted-foreground">Loading project…</div>
	{:else if project}
		<div class="mx-auto max-w-6xl space-y-8 p-4 sm:p-6">
			...
			{#if creatingConversation}
				<form onsubmit={(event) => { event.preventDefault(); void createConversation(); }} class="space-y-4 rounded-xl border border-primary/30 bg-card p-4 shadow-sm">
			...
			<div class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.65fr)]">
				<section>
					...
					<div class="overflow-hidden rounded-xl border border-border bg-card">{#if taskList.length === 0}<p class="p-5 text-sm text-muted-foreground">No tasks in this project yet.</p>{:else}{#each taskList.slice(0, 8) as task (task.id)}<a href="/tasks/{task.id}" class="flex items-center gap-3 border-b border-border/70 px-4 py-3 last:border-0 hover:bg-accent/30"><span class="h-2 w-2 shrink-0 rounded-full {taskDot(task.status)}"></span><span class="min-w-0 flex-1"><span class="block truncate text-sm">{task.title}</span><span class="text-[11px] text-muted-foreground">{task.status.replaceAll('_', ' ')} · {timeAgo(task.updated_at)}</span></span><span class="text-muted-foreground">→</span></a>{/each}{/if}</div>
				</section>
