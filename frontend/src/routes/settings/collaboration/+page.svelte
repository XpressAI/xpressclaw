<script lang="ts">
	...
	async function run(action: Action) {
		if (busy || ((action === 'install' || action === 'restart' || action === 'upgrade') && !form?.enabled)) return;
		busy = action;
		error = '';
		notice = '';
		try {
		...
		<section class="rounded-lg border border-border bg-card p-4 space-y-3">
			...
			<div class="flex flex-wrap gap-2">
				...
				{#each ['start', 'stop', 'restart', 'upgrade'] as action}
					<button class="rounded-md border border-border px-3 py-2 text-xs capitalize disabled:opacity-50" disabled={busy !== null || ((action === 'restart' || action === 'upgrade') && !form.enabled)} onclick={() => run(action as Action)}>
