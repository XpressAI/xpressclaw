...
<div class="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
	...
	{#if desktop}
		<section class="rounded-xl border border-border bg-card p-4 shadow-sm">
			...
			<p class="mt-1 text-xs text-muted-foreground">This Desktop window uses one selected profile at a time. Switching profiles clears the current browser session before navigation so instances on different ports cannot receive each other’s cookies. The automatic local sidecar continues running when you connect remotely. Credentials stay in the operating-system keychain. Automatic keychain login is local-sidecar-only; remote profiles use the normal browser login.</p>
