...
/// Hook configuration for pre/post message processing.
///
/// Hooks allow agents to configure custom processing steps that run
/// before or after each message (e.g. linting, secret obfuscation,
...
/// Legacy minimal config template retained for downstream API compatibility.
///
/// The CLI now renders instance-local paths when it initializes a new instance
/// and therefore does not use this constant.
...
"#;
    ...
    #[test]
    fn retired_shared_runner_image_recognizes_a_dsh_backend_alias() {
        let yaml = r#"
agents:
  - name: deepseek
    backend: dsh-acp
