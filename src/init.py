...
def run_init(backend: str = "claude-code", force: bool = False) -> None:
    ...
    if config_path.exists() and not force:
        click.echo(click.style("⚠️  xpressai.yaml already exists.", fg="yellow"))
    ...
    if gpu_info["available"]:
        ...
        click.echo(click.style("⚠️  No GPU detected.", fg="yellow"))
        if backend == "local":
    ...
    if backend == "claude-code":
        ...
        click.echo("  2. Download the model: xpressai model download")
        click.echo("  3. Start the runtime: xpressai up")
    else:
